...
public class BukkitUpdater extends Updater {
	...
	protected boolean read() {
		...
		try {
			...
			if (array.size() == 0) {
				getPlugin().getLogger().warning("The updater could not find any files for the project id " + this.getId());
				this.setResult(UpdateResult.FAIL_BADID);
				return false;
			}
			...
			getPlugin().getLogger().warning("The updater could not contact dev.bukkit.org for updating.");
			getPlugin().getLogger()
			        .warning(
